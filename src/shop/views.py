...
from .models import Product, Category, CartProduct, Customer, Error
...
def write_the_error_about_unsend_mail(owner, customer_orders, price):
    goods =[]
    for item in customer_orders:
        goods.append('Назва ')
        goods.append(item.product)
        goods.append(' Ціна ')
        goods.append(item.price)
        goods.append(' Кількість ')
        goods.append(item.quantity)
        goods.append(' Загальна вартість ')
        goods.append(item.total)

    error = Error.objects.create(
        error=f"""Your mail to {owner} wasn't sended, look for his items in cart and try to call him {owner.phone_number}! 
        {goods}! The total price are {price} UAH!"""
    )
    error.save()
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            try:
                ...
                write_the_error_about_unsend_mail(owner, customer_orders, price)
            finally:   
